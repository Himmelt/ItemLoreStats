...
public class Rarity {
    public String get(double dropChance) {
        ...
        for (int i = 0; i < ItemLoreStats.plugin.getConfig().getConfigurationSection("rarity").getKeys(false).size(); ++i) {
            Double rarePercent = Double.parseDouble(ItemLoreStats.plugin.getConfig().getConfigurationSection("rarity").getKeys(false).toString().split(",")[i].replaceAll("\\[", "").replaceAll("\\]", "").trim());
